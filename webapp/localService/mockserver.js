sap.ui.define(
	...
	function (MockServer, AjaxHelper, Log) {
		...
		var aUrlPrefixMatches = document.location.pathname.match(
			/(.*)\/test-resources\//
		);
		var sUrlPrefix = (aUrlPrefixMatches && aUrlPrefixMatches[1]) || '';
		var sImgDirectoryPath = '/localService/images/';
				...
				for (var property in oDataSources) {
					if (
						...
						if (
							...
							if (
								typeof dataSource.type === 'undefined' ||
								...
								sMockServerPath = sMetadataUrl.slice(
									0,
									sMetadataUrl.lastIndexOf('/') + 1
								);
								...
								if (property === 'mainService') {
									oMockServer.attachAfter(
										MockServer.HTTPMETHOD.GET,
										function (oEvent) {
											var oParameters =
												oEvent.getParameters();

											if (
												oParameters.oFilteredData &&
												Array.isArray(
													oParameters.oFilteredData
														.results
												)
											) {
												oParameters.oFilteredData.results.forEach(
													function (oProduct) {
														oProduct.ProductPictureURL =
															getImagePath(
																oProduct.ProductPictureURL
															);
													}.bind(this)
												...
												oParameters.oEntry &&
												typeof oParameters.oEntry
													.ProductPictureURL ===
													'string'
											) {
												oParameters.oEntry.ProductPictureURL =
													getImagePath(
														oParameters.oEntry
															.ProductPictureURL
													);
								...
								if (oUriParameters.get('sap-client')) {
									dataSource.uri = dataSource.uri.concat(
										'&sap-client=' +
											oUriParameters.get('sap-client')
									);
								}
								var rRegEx = dataSource.uri;
								if (dataSource.type !== 'MockRegEx') {
									rRegEx = new RegExp(
										MockServer.prototype._escapeStringForRegExp(
											dataSource.uri
										) + '([?#].*)?'
									);
								}
							...
							oMockServer.start();
							Log.info(
								'Running the app with mock data for ' + property
							);
						}
					}
				}
